- [Python基础教程(一)](#python基础教程一)
- [1. 基本概念](#1-基本概念)
  - [1.1 四种类型](#11-四种类型)
  - [1.2 字符串](#12-字符串)
  - [1.3 标识符的命名](#13-标识符的命名)
  - [1.4 对象](#14-对象)
  - [1.5 逻辑行和物理行](#15-逻辑行和物理行)
  - [1.6 缩进](#16-缩进)
- [2 运算符与表达式](#2-运算符与表达式)
  - [2.1 运算符与其用法](#21-运算符与其用法)
  - [2.2 运算符优先级](#22-运算符优先级)
  - [2.3 输出](#23-输出)
- [3. 控制流](#3-控制流)
  - [3.1 if 语句](#31-if-语句)
  - [3.2 while语句](#32-while语句)
  - [3.3 break语句](#33-break语句)
  - [3.4 continue语句](#34-continue语句)
- [4. 函数](#4-函数)
  - [4.1 函数形参](#41-函数形参)
  - [4.2 局部变量](#42-局部变量)
  - [4.3 默认参数](#43-默认参数)
  - [4.4 关键参数](#44-关键参数)
  - [4.5 return 语句](#45-return-语句)
  - [4.6 文档字符串](#46-文档字符串)
- [5. 模块](#5-模块)
  - [5.1 字节编译的.pyc文件](#51-字节编译的pyc文件)
  - [5.2 from .. import](#52-from--import)
  - [5.3 模块的__name__](#53-模块的__name__)
...
Python假定每个物理行都对应着一个逻辑行。
    
例如：print( "Hello World" ) 就是一个物理行，Python希望每行只有一个语句，因为这样看起来更加易读。
...
|运算符|名称|例子|
|----|----|----|
|+|**加**两个对象相加|加法，如3 + 5得到8,字符也可以相加'a' + 'b'得到'ab'|
|-|**减**一个数减去另一个数|5 - 2得到3|
...
|&VerticalLine;|	按位或	|数的按位或 5 &VerticalLine; 3得到7。二进制101&11，变成111，如果两个值相应的位置有一个是1,那么该结果就是1，也就是如果都是0，该结果就是0，101和11没有都是0，所以111|
...
- break：跳出整个循环
- continue：跳出本次循环，继续执行下一次循环 希望大家牢记。

# 4. 函数
函数通过```def```定义。def关键字后跟函数的标识符名称，然后跟一对圆括号，括号之内可以包含一些变量名，该行以冒号结尾；接下来是一块语句，即函数体。
```
def sumOf(a, b):
    return a + b
```
...
__doc__用来访问模块，类声明或者函数的声明中第一个未被赋值的字符串，可以是被""" ""","" "",' ',括起来的，访问的方法就是obj.__doc__，obj是这个模块，类或者函数的名称，可以print出来。
```
def func():
    '''This is self-defined function
	Do nothing
	'''
    pass# 在Python 中，pass 是一个空语句，为了保持程序结构的完整\性。 一般情况下，pass 不做任何事情，被用作占位符。
...
4、\_\_name\_\_就是标识模块的名字的一个系统变量。这里分两种情况：假如当前模块是主模块（也就是调用其他模块的模块），那么此模块名字就是__main__，通过if判断这样就可以执行“\_\_main\_\_:”后面的主函数内容；假如此模块是被import的，则此模块名字为文件名字（不加后面的.py），通过if判断这样就会跳过“\_\_main\_\_:”后面的内容。
...
由于此程序是主程序，所以一旦执行
因为```__name__ == '__main__'```，进入if，所以会执行test函数。
